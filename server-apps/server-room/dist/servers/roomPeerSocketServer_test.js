...
import { defaultPeerSocketServerSecurityMap, RoomPeerSocketServer } from './roomPeerSocketServer.js';
...
describe('RoomPeerSocketServer', () => {
    let roomServer;
    let config;
    let securityMap = defaultPeerSocketServerSecurityMap;
    let mockWSServer;
    let peerSocketServer;
    let testResults = {};
    beforeAll(async () => {
        jest.clearAllMocks();
        ...
        mockWS.addEventListener("message", (event) => {
            ...
            switch (msgIn.type) {
                case payloadTypeServer.registerPeerResult: {
                    ...
                    let registerPeerResultMsg = msgIn;
