...
import { EventTypes } from "./conferenceEvents.js";
...
export class ConferenceClient {
    ...
    async waitRegisterConnection(participantGroup: string, conferenceGroup: string, username: string, displayName: string, authToken: string, clientData: {}): Promise<RegisterResultMsg> {
        ...
        if (resultMsg.error) {
            throw new Error(`Failed to register connection: ${resultMsg.error}`);
        }

        return resultMsg as RegisterResultMsg;
    }
    ...
    async waitTryRegister(): Promise<boolean> {
        console.log("waitTryRegister");
        const timeout = (this.config.conf_socket_register_timeout_secs ?? 30) * 1000;
        const resultMsg = await this.waitForMessage(CallMessageType.registerResult, timeout) as RegisterResultMsg;
    ...
    async publishTracks(tracks: MediaStreamTrack[], why: string) {
        ...
        //publish the tracks
        if (this.roomManager) {
            await this.roomManager.publishTracks(tracks);
        }
    ...
    unPublishTracks(tracks: MediaStreamTrack[]) {
        ...
        if (this.roomManager) {
            this.roomManager.unPublishTracks(tracks);
        }
    }
    ...
    broadCastTrackInfo() {
        ...
        if (this.roomManager) {
            this.roomManager.broadCastTrackInfo();
        }
    }
    ...
    isBroadcastingVideo() {
        ...
        return this.roomManager.isBroadcastingVideo();
    }
    ...
    isBroadcastingAudio() {
        ...
        return this.roomManager.isBroadcastingAudio();
    }
    ...
    muteParticipantTrack(participantId: string, audioMuted: boolean, videoMuted: boolean) {
        ...
        if (this.roomManager) {
            ...
            this.roomManager.muteParticipantTrack(peerId, audioMuted, videoMuted);
        }
    ...
    waitCreateConferenceRoom(args: CreateConferenceParams) {
        console.log(`waitCreateConferenceRoom externalId: ${args.externalId}`);
        if (!this.isRegistered()) {
        ...
        if (!this.createConferenceRoom(args)) {
            throw new Error("Failed to send createConferenceRoom message.");
        }
        return this.waitForMessage(CallMessageType.createConfResult, 5000) as Promise<CreateConfResultMsg>;
    }

    waitJoinConferenceRoom(args: JoinConferenceParams) {
        console.log(`waitJoinConferenceRoom conferenceId: ${args.conferenceId}`);
        if (!this.isRegistered()) {
            throw new Error("Connection not registered.");
        }
        if (!this.joinConferenceRoom(args)) {
            throw new Error("Failed to send joinConferenceRoom message.");
        }
        return this.waitForMessage(CallMessageType.joinConfResult, 5000) as Promise<JoinConfResultMsg>;
    }
    ...
    async onInviteReceived(message: InviteMsg) {
        console.log("onInviteReceived()", message);
        if (this.isInConference()) {
    ...
    sendConferencePong(conferenceId: string) {
        ...
        this.roomManager.roomPong(this.conference.roomId);
    }
    ...
    private async onConferenceReady(message: ConferenceReadyMsg) {
        ...
        try {
            this.roomManager = new RoomManager(this.conference, this.localParticipant, this.username);
            this.setupMediaManagerEvents();

            await this.roomManager.initialize(message.data.roomURI, message.data.roomRtpCapabilities);
            await this.roomManager.connectAndJoin();
    ...
    private setupMediaManagerEvents() {
        if (!this.roomManager) return;

        this.roomManager.setEventHandler(async (eventType: string, payload: IMsg) => {
            switch (eventType) {
                ...
                case EventTypes.participantJoined: {
                    const { peer, roomId } = payload.data;
                    let participant = this.conference.participants.get(peer.trackingId);
                    if (participant) {
                        console.error(`participant already in local conference: ${peer.trackingId}`);
                        return;
                    }
                ...
                case EventTypes.participantNewTrack: {
                    const { peer, track } = payload.data;
                    let participant = this.conference.participants.get(peer.trackingId);
                    if (!participant) {
                        console.error(`participant not found for new track: ${peer.trackingId}`);
                        return;
                    }
                ...
                case EventTypes.participantTrackInfoUpdated: {
                    const { peer } = payload.data;
                    let participant: Participant = (this.localParticipant.participantId === peer.trackingId)
                        ? this.localParticipant
                        : this.conference.participants.get(peer.trackingId);
